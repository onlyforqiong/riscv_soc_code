...
def chg_ld_script(app_type):
    if app_type == 'flash' : 
        os.system("sed -i 's|-T $(AM_HOME)/scripts/linker|-T $(AM_HOME)/scripts/linker_flash|' $AM_HOME/scripts/" + APP_ARCH + ".mk")
    else :
        os.system("sed -i 's|-T $(AM_HOME)/scripts/linker_flash|-T $(AM_HOME)/scripts/linker|' $AM_HOME/scripts/" + APP_ARCH + ".mk")
...
if APP_TYPE == 'flash':
    ...
    if APP_NAME != 'rtthread':
        ...
        os.system('cp main.c bsp/qemu-riscv-virt64/applications/')
        os.chdir('bsp/qemu-riscv-virt64/')
